        ...
        <div className="comment-container">
           ...
           <div>
                ...
                <div className="comment-reaction">
                    ...
                    <button className="comment-reply-button" onClick={() => setShowReplyInput(!showReplyInput)}>Reply</button>
                </div>
                {showReplyInput && (
                    <div>
                    <input
                ...
                {commentData.replies.map(reply => (
                    <div key={reply._id} style={{ marginLeft: '20px' }}>
                        <p><strong>{reply.user.displayName}</strong>: {reply.text}</p>
                        <i className={`fa ${reply.likes.includes(userData?.id) ? 'fa-solid' : 'fa-regular'} fa-thumbs-up`} 
                        onClick={() => userData && handleLikeReply(reply._id)}></i> ({reply.likes.length})
                        <i className={`fa ${reply.dislikes.includes(userData?.id) ? 'fa-solid' : 'fa-regular'} fa-thumbs-down`} 
                        onClick={() => userData && handleDislikeReply(reply._id)}></i> ({reply.dislikes.length})
                    </div>
                    ))}
                </div>
            </div>
