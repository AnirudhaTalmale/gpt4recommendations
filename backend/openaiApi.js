  ...
  try {
    const response = await axios.get(`https://www.googleapis.com/customsearch/v1`, {
      params: {
        ...
        q: `site:amazon.in ${title} by ${author}`,
        num: 1
      }
    });
    ...
    if (response.data.items?.length) {
      const filteredBooks = response.data.items.filter(item => {
        ...
        return (itemTitleLower.includes(searchTitleLower) || searchTitleLower.includes(itemTitleLower)) 
          && item.volumeInfo.language === 'en' // Check language here to reduce iterations in the next filters
          && item.volumeInfo.industryIdentifiers?.some(id => id.type === 'ISBN_13' || id.type === 'ISBN_10'); // Check for ISBN presence
      });

      // At this point, filteredBooks only includes English books with an ISBN
      const book = filteredBooks.find(item => item.accessInfo.embeddable) || filteredBooks[0];
      if (book) {
